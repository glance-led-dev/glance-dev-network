...
#
# One keyless lookup: zippopotam.us, zip -> city, latitude, longitude, for the
# city name and the sunrise/sunset math. The clock's offset comes from the
# four-way US time zone dropdown, resolved locally with daylight saving, so
# no time API and one render per zone.
...
US_ABBR = {"EASTERN": ["EST", "EDT"], "CENTRAL": ["CST", "CDT"],
           "MOUNTAIN": ["MST", "MDT"], "PACIFIC": ["PST", "PDT"]}

def zone_label(ctx, off):
    """EDT while daylight saving is in force, EST otherwise; the raw offset
    for anything the table does not know."""
    zone = str(ctx.inputs.get("timezone", "EASTERN")).strip().upper()
    if zone in US_ABBR:
        std = US_ZONES[zone]
        return US_ABBR[zone][1] if int(off * 60) != std else US_ABBR[zone][0]
    return utc_str(off)

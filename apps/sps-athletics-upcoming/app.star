...
def draw_sport_icon(c, title, cx, cy):
    """A ball/equipment icon centered on (cx, cy), matched by keyword
    against the sport title — GDN's bundled icon set (gdn/data/icons.json)
    is generic UI glyphs (arrows, sun, bell, ...), nothing sports-related.
    Sized for the gap between the SPS logo and the center date/score text
    (it's genuinely empty space — the logo is 24px, the icon has ~26px of
    width to work with before the center text's own worst-case left edge).
    Every sport this school fields gets one; anything genuinely unmatched
    just skips the icon rather than force a bad one.

    Every sport now uses a pre-rendered 18x18 icon instead of primitives.
    Most come from open icon sets — game-icons.net (free, CC BY 3.0, icons
    by Delapouite — attribution kept here per the license), Google's
    Material Symbols (Apache 2.0: cross country, skiing, baseball's ball,
    crew's rower, and wrestling's single figure — the closest single-
    person combat pose available, martial arts' kick), and OpenMoji (CC
    BY-SA 4.0: field hockey's stick+ball, kept in its original full color
    rather than recolored). Each source SVG had any opaque background
    square it shipped with stripped, then was rendered at high super-
    sampling and downsampled with a gamma-boosted alpha channel to re-
    sharpen the edges the downsample softened — the raw vector art,
    straight from the SVG, blurs into gray smudge once shrunk to 18px
    without that cleanup pass. Squash is the one exception: no squash-
    specific icon exists anywhere (checked all of the above plus Tabler),
    and the closest single-racquet shape available — badminton's, from
    Material Symbols — reads as badminton, not squash, so this is instead
    a small hand-drawn dark ball with a highlight and a yellow marking
    dot, the way a real (double-yellow-dot) squash ball actually looks."""
    t = title.upper()

    vector_icon = None
    if "SOCCER" in t:
        vector_icon = "sport-icons/soccer.png"
    elif "BASKETBALL" in t:
        vector_icon = "sport-icons/basketball.png"
    elif "FOOTBALL" in t:
        vector_icon = "sport-icons/football.png"
    elif "VOLLEYBALL" in t:
        vector_icon = "sport-icons/volleyball.png"
    elif "GOLF" in t:
        vector_icon = "sport-icons/golf.png"
    elif "TENNIS" in t:
        vector_icon = "sport-icons/tennis.png"
    elif "ICE HOCKEY" in t:
        vector_icon = "sport-icons/ice-hockey.png"
    elif "FIELD HOCKEY" in t:
        vector_icon = "sport-icons/field-hockey.png"
    elif "CREW" in t:
        vector_icon = "sport-icons/crew.png"
    elif "CROSS COUNTRY" in t or "TRACK" in t:
        vector_icon = "sport-icons/cross-country.png"
    elif "SKIING" in t:
        vector_icon = "sport-icons/skiing.png"
    elif "BASEBALL" in t or "SOFTBALL" in t:
        vector_icon = "sport-icons/baseball.png"
    elif "SQUASH" in t:
        vector_icon = "sport-icons/squash.png"
    elif "WRESTLING" in t:
        vector_icon = "sport-icons/wrestling.png"

    if vector_icon != None:
        c.image(vector_icon, cx - 9, cy - 9, 18, 18)

def card(c, ctx, sport_label, opponent, center_lines, accent = BIG_RED, small_line = None, icon_title = None):
    ...
    # The sport icon lives in its own slot between the SPS logo and the
    # center text, not squeezed beside the score — there's real empty
    # space there (logo ends at x=26; the center text's own worst-case
    # left edge is ~x=66), room enough for a proper 16-18px icon.
    if icon_title != None:
        draw_sport_icon(c, icon_title, 44, y0 + 11)
    cx = c.width // 2
    ty = y0 + 1
...
def render_upcoming_slot(c, ctx, slot_index):
    ...
    if has_result(e):
        ...
        if result["team_score"] != None and result["opponent_score"] != None:
            ...
            lines = [when, "FINAL"]
        card(c, ctx, header, opponent, lines, accent = accent,
             small_line = result_note(result), icon_title = e["sport"]["title"])
    else:
        card(c, ctx, e["sport"]["title"].upper(), opponent,
             [when, e["time"].upper()], small_line = venue_line(e),
             icon_title = e["sport"]["title"])

...
#
# Top 4 pitching leaders for a chosen category (ERA/K/Wins/Saves/IP/Games/WHIP/K9),
...
# same design, pitching stats instead of batting.
#
# Pages in this SDK are a fixed list declared in the manifest, so all 8 category
# pages always exist; each one's own "show this category" toggle just decides
# whether it renders real stats or a "disabled" placeholder.
...
def wins(c, ctx):
    draw_leaderboard(c, ctx, "wins", "WINS")

def saves(c, ctx):
    draw_leaderboard(c, ctx, "saves", "SAVES")

def ip(c, ctx):
    draw_leaderboard(c, ctx, "inningsPitched", "INNINGS PITCHED")

def games(c, ctx):
    draw_leaderboard(c, ctx, "gamesPlayed", "GAMES")

def whip(c, ctx):
    draw_leaderboard(c, ctx, "walksAndHitsPerInningPitched", "WHIP")

def k9(c, ctx):
    draw_leaderboard(c, ctx, "strikeoutsPer9Inn", "K PER 9 INN")

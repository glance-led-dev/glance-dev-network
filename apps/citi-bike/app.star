...
PAD = 6                # scroll edge padding: content lives in x 6 .. w-7
...
# 192 (scroll). One row of air at the top and one at the bottom -- the owner's
# "save some space top to bottom" -- and the 10x15_outline number lands exactly
# centred in the 17-row window between the label above it (ink ends at 11) and
# the bar below it (ink starts at 29): 15 rows of digit, one blank row each
# side. Name ink 1..5, labels 7..11, numbers 13..27, bar 29..30, row 31 black.
W_NAME_Y = 1
W_LABEL_Y = W_NAME_Y + FONT_INK[LABEL_FONT] + GAP           # 7
W_NUM_Y = W_LABEL_Y + FONT_INK[LABEL_FONT] + GAP            # 13
W_NUM_FONT = "10x15_outline"
W_BAR_Y = W_NUM_Y + FONT_INK[W_NUM_FONT] + GAP              # 29
...
def wide(c, name, total, ebikes, docks, renting, returning):
    ...
    # Three equal columns filling everything to the right of the bike. Centring
    # the content of each column on its own centre line is what pulled the
    # figures closer together: the old layout hung them off fixed left edges
    # 56 px apart and the group drifted right as the counts got shorter.
    band0 = left + BIKE_W + 2
    cw = (right - band0 + 1) // 3
    cx0 = band0 + cw // 2
    ...
    bar(c, left, right, W_BAR_Y, BAR_H, total, ebikes, docks)

# ported-apiimage — scaffolded by `gdn translate` from a Pixlet/tronbyt app.
#
...
#   - render.Box  ->  c.rect(x0, y0, x1, y1, fill=) for the background
#   - render.Image  ->  re-export the PNG into this folder, list it in assets:, c.image(...)
#   - render.Marquee  ->  static c.text() for now (scrolling needs the animation phase)
#   - render.Root  ->  the page itself — becomes def main(c, ctx)
#   - render.Row  ->  lay children out left-to-right by hand (track an x cursor)
#   - render.Stack  ->  draw children in order (later on top)
#   - render.Text  ->  c.text(s, x, y, font=, color=, align=)
#   - render.WrappedText  ->  split into lines and c.text() each

def main(c, ctx):
    # NOT PORTED (honest scoreboard entry). This app's entire output IS a single
    # image fetched at runtime from a user-supplied URL (http.get -> render.Image
    # full-screen). There is no data/logic to keep and no procedural picture to
    # draw: GDN v1 has no runtime http, and c.image() can only paste a PNG that is
    # bundled in the app folder and declared in the manifest — not a live URL. So
    # unlike the analog-clock/big-clock ports (whose images were decorative and
    # could be redrawn), there is nothing to render offline. Left as the scaffold.
    c.fill("black")
    c.text("API IMAGE", c.width // 2, 3, font = "5x7", color = "gray", align = "center")
    c.text("NEEDS HTTP", c.width // 2, 13, font = "5x7", color = "red", align = "center")
    c.text("NOT PORTED", c.width // 2, 23, font = "4x5", color = "gray", align = "center")
